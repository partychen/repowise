...
The first sync initializes `.review/config.yaml` and saves the repository binding.
...
3. For each `pending_tasks` entry, resolve its `path` relative to `$Target`. Read
...
5. Save the response at a task-specific path under `.review/local`, then execute:
...
`TASK_PATH` and `RESPONSE_PATH` must be resolved relative to the target, not the
Skill installation or an unrelated shell cwd. `propose` paths are normal CLI file
